        ...
        <div class="card-body" :class="store.properties.mask_adult > 30 ? 'hightlight': 'normal'">
            <div class="d-flex align-items-end">
                <h4 class="card-title mt-3">{{store.properties.name}}
                    <button type="button" v-if="store.properties.note.length > 1" id="toggleModal" class="small text-white bg-danger" @click="clickHandler">
                        註
                    </button>
                </h4>
            </div>
